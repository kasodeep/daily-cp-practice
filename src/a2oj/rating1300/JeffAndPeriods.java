    ...
    public static void main(String[] args) {
        try {
            ...
            for (int i = 0; i < n; ) {
                ...
                if (i != n - 1 && pairs[i + 1].x == temp)
                    diff = pairs[i + 1].y - pairs[i].y;

                i += 1;
                while (i < n && pairs[i].x == temp) {
                    if (pairs[i].y - pairs[i - 1].y != diff) {
                        flag = false;
                        break;
                    }
                    i++;
                ...
                if (flag) map.add(new Pair(temp, diff));
            }
